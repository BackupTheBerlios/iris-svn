...
CalMorphTargetMixer::CalMorphTargetMixer ():m_pModel (0)
{
}
...
float CalMorphTargetMixer::getCurrentWeightBase ()
{
  ...
  std::vector < float >::iterator iteratorCurrentWeight =
    m_vectorCurrentWeight.begin ();
...
bool CalMorphTargetMixer::create (CalModel * pModel)
{
  ...
  if (pModel->getCoreModel ()->getCoreMorphAnimationCount () != 0)
      {
        int morphAnimationCount =
          pModel->getCoreModel ()->getCoreMorphAnimationCount ();
        ...
        std::vector < float >::iterator iteratorCurrentWeight =
          m_vectorCurrentWeight.begin ();
        std::vector < float >::iterator iteratorEndWeight =
          m_vectorEndWeight.begin ();
        std::vector < float >::iterator iteratorDuration =
          m_vectorDuration.begin ();
        while (iteratorCurrentWeight != m_vectorCurrentWeight.end ())
            {
  ...
  while (iteratorCurrentWeight != m_vectorCurrentWeight.end ())
      {
        if (deltaTime >= (*iteratorDuration))
        ...
        else
            {
              (*iteratorCurrentWeight) +=
                ((*iteratorEndWeight) -
                 (*iteratorCurrentWeight)) * deltaTime / (*iteratorDuration);
  ...
  while (morphAnimationID < getMorphTargetCount ())
      {
        ...
        std::vector < int >&vectorCoreMeshID =
          pCoreMorphAnimation->getVectorCoreMeshID ();
        std::vector < int >&vectorMorphTargetID =
          pCoreMorphAnimation->getVectorMorphTargetID ();
        size_t meshIterator = 0;
        while (meshIterator < vectorCoreMeshID.size ())
            {
              std::vector < CalSubmesh * >&vectorSubmesh =
                m_pModel->getMesh (vectorCoreMeshID[meshIterator])->
                getVectorSubmesh ();
