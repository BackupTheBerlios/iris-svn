...
CalMesh::CalMesh ():m_pModel (0), m_pCoreMesh (0)
{
}
...
void CalMesh::destroy ()
{
  ...
  for (iteratorSubmesh = m_vectorSubmesh.begin ();
       iteratorSubmesh != m_vectorSubmesh.end (); ++iteratorSubmesh)
      {
  ...
  for (submeshId = 0; submeshId < (int) m_vectorSubmesh.size (); ++submeshId)
      {
        ...
        coreMaterialThreadId =
          m_vectorSubmesh[submeshId]->getCoreSubmesh ()->
          getCoreMaterialThreadId ();

        // get the core material id for the given set id in the material thread
        int coreMaterialId;
        coreMaterialId =
          m_pModel->getCoreModel ()->getCoreMaterialId (coreMaterialThreadId,
                                                        setId);
