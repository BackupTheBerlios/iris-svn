...
bool CalMixer::blendCycle (int id, float weight, float delay)
{
  ...
  if (pAnimation == 0)
      {
        ...
        CalCoreKeyframe *lastKeyframe =
          coreTrack->getCoreKeyframe (coreTrack->getCoreKeyframeCount () - 1);
        ...
        if (lastKeyframe->getTime () < pCoreAnimation->getDuration ())
            {
              ...
              for (itr = listCoreTrack.begin (); itr != listCoreTrack.end ();
                   ++itr)
                  {
                    coreTrack = *itr;

                    CalCoreKeyframe *firstKeyframe =
                      coreTrack->getCoreKeyframe (0);
                    CalCoreKeyframe *newKeyframe = new CalCoreKeyframe ();

                    newKeyframe->setTranslation (firstKeyframe->
                                                 getTranslation ());
  ...
  if (pAnimation->getType () != CalAnimation::TYPE_CYCLE)
      {
        CalError::setLastError (CalError::INVALID_ANIMATION_TYPE, __FILE__,
                                __LINE__);
        return false;
      }
...
bool CalMixer::create (CalModel * pModel)
{
  ...
  m_vectorAnimation.insert (m_vectorAnimation.begin (),
                            (std::vector <
                             CalAnimation * >::size_type) coreAnimationCount,
                            0);
  ...
  if (m_animationDuration == 0.0f)
  ...
  else
      {
        ...
        if (m_animationTime >= m_animationDuration)
            {
              m_animationTime =
                (float) fmod (m_animationTime, m_animationDuration);
            }
      }
  ...
  while (iteratorAnimationAction != m_listAnimationAction.end ())
      {
        ...
        if ((*iteratorAnimationAction)->update (deltaTime))
        ...
        else
            {
              ...
              iteratorAnimationAction =
                m_listAnimationAction.erase (iteratorAnimationAction);
            }
      }
        ...
        if ((*iteratorAnimationCycle)->update (deltaTime))
            {
              ...
              if ((*iteratorAnimationCycle)->getState () ==
                  CalAnimation::STATE_SYNC)
                  {
                    accumulatedWeight +=
                      (*iteratorAnimationCycle)->getWeight ();
                    accumulatedDuration +=
                      (*iteratorAnimationCycle)->getWeight () *
                      (*iteratorAnimationCycle)->getCoreAnimation ()->
                      getDuration ();
                  }
        ...
        else
            {
              ...
              iteratorAnimationCycle =
                m_listAnimationCycle.erase (iteratorAnimationCycle);
            }
      }
...
void CalMixer::updateSkeleton ()
{
  ...
  std::list < CalAnimationAction * >::iterator iteratorAnimationAction;
  for (iteratorAnimationAction = m_listAnimationAction.begin ();
       iteratorAnimationAction != m_listAnimationAction.end ();
       ++iteratorAnimationAction)
      {
        ...
        for (iteratorCoreTrack = listCoreTrack.begin ();
             iteratorCoreTrack != listCoreTrack.end (); ++iteratorCoreTrack)
            {
              ...
              (*iteratorCoreTrack)->getState ((*iteratorAnimationAction)->
                                              getTime (), translation,
                                              rotation);

              // blend the bone state with the new state
              pBone->blendState ((*iteratorAnimationAction)->getWeight (),
  ...
  std::list < CalAnimationCycle * >::iterator iteratorAnimationCycle;
  for (iteratorAnimationCycle = m_listAnimationCycle.begin ();
       iteratorAnimationCycle != m_listAnimationCycle.end ();
       ++iteratorAnimationCycle)
      {
        // get the core animation instance
        CalCoreAnimation *pCoreAnimation;
        ...
        if ((*iteratorAnimationCycle)->getState () ==
            CalAnimation::STATE_SYNC)
            {
              if (m_animationDuration == 0.0f)
                  ...
                  {
                    animationTime =
                      m_animationTime * pCoreAnimation->getDuration () /
                      m_animationDuration;
                  }
            }
        else
        ...
        for (iteratorCoreTrack = listCoreTrack.begin ();
             iteratorCoreTrack != listCoreTrack.end (); ++iteratorCoreTrack)
            {
              ...
              CalVector translation;
              CalQuaternion rotation;
              (*iteratorCoreTrack)->getState (animationTime, translation,
                                              rotation);

              // blend the bone state with the new state
              pBone->blendState ((*iteratorAnimationCycle)->getWeight (),
                                 translation, rotation);
            }
      }
