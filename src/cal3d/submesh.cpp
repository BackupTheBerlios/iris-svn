...
CalSubmesh::CalSubmesh ():m_pCoreSubmesh (0)
{
}
...
bool CalSubmesh::create (CalCoreSubmesh * pCoreSubmesh)
{
  ...
  m_vectorMorphTargetWeight.reserve (m_pCoreSubmesh->
                                     getCoreSubMorphTargetCount ());
  m_vectorMorphTargetWeight.resize (m_pCoreSubmesh->
                                    getCoreSubMorphTargetCount ());
  int morphTargetId;
  for (morphTargetId = 0;
       morphTargetId < m_pCoreSubmesh->getCoreSubMorphTargetCount ();
       ++morphTargetId)
      {
  ...
  if (m_pCoreSubmesh->getSpringCount () > 0)
      {
        ...
        m_vectorvectorTangentSpace.reserve (m_pCoreSubmesh->
                                            getVectorVectorTangentSpace ().
                                            size ());
        m_vectorvectorTangentSpace.resize (m_pCoreSubmesh->
                                           getVectorVectorTangentSpace ().
                                           size ());
        ...
        std::vector < CalCoreSubmesh::Vertex > &vectorVertex =
          m_pCoreSubmesh->getVectorVertex ();

        // copy the data from the core submesh as default values
        int vertexId;
        for (vertexId = 0; vertexId < m_pCoreSubmesh->getVertexCount ();
             ++vertexId)
            {
              // copy the vertex data
              m_vectorVertex[vertexId] = vectorVertex[vertexId].position;
              m_vectorPhysicalProperty[vertexId].position =
                vectorVertex[vertexId].position;
              m_vectorPhysicalProperty[vertexId].positionOld =
                vectorVertex[vertexId].position;
...
std::vector < std::vector < CalSubmesh::TangentSpace >
  >&CalSubmesh::getVectorVectorTangentSpace ()
{
...
std::vector < CalSubmesh::PhysicalProperty >
  &CalSubmesh::getVectorPhysicalProperty ()
{
...
bool CalSubmesh::enableTangents (int mapId, bool enabled)
{
  ...
  m_vectorvectorTangentSpace[mapId].reserve (m_pCoreSubmesh->
                                             getVertexCount ());
  m_vectorvectorTangentSpace[mapId].resize (m_pCoreSubmesh->
                                            getVertexCount ());

  // get the tangent space vector of the core submesh
  std::vector < CalCoreSubmesh::TangentSpace > &vectorTangentSpace =
    m_pCoreSubmesh->getVectorVectorTangentSpace ()[mapId];
  ...
  for (vertexId = 0; vertexId < m_pCoreSubmesh->getVertexCount (); vertexId++)
      {
        ...
        m_vectorvectorTangentSpace[mapId][vertexId].tangent =
          vectorTangentSpace[vertexId].tangent;
        m_vectorvectorTangentSpace[mapId][vertexId].crossFactor =
          vectorTangentSpace[vertexId].crossFactor;
      }
...
void CalSubmesh::setLodLevel (float lodLevel)
{
  ...
  int vertexId;
  for (vertexId = vectorVertex.size () - 1; vertexId >= m_vertexCount;
       vertexId--)
      {
        ...
        for (vertexId = 0; vertexId < 3; ++vertexId)
            {
              ...
              while (collapsedVertexId >= m_vertexCount)
                collapsedVertexId =
                  vectorVertex[collapsedVertexId].collapseId;
