                                  ...
                                  CalCoreAnimation * pCoreAnimation)
{
  if (strFilename.size () >= 3
      && stricmp (strFilename.substr (strFilename.size () - 3, 3).c_str (),
                  Cal::ANIMATION_XMLFILE_MAGIC) == 0)
  ...
  if (!file)
      {
        CalError::setLastError (CalError::FILE_CREATION_FAILED, __FILE__,
  ...
  // write magic tag
  if (!CalPlatform::
      writeBytes (file, &Cal::ANIMATION_FILE_MAGIC,
                  sizeof (Cal::ANIMATION_FILE_MAGIC)))
      {
        CalError::setLastError (CalError::FILE_WRITING_FAILED, __FILE__,
                                __LINE__, strFilename);
        return false;
      }
  ...
  // get core track list
  std::list < CalCoreTrack * >&listCoreTrack =
    pCoreAnimation->getListCoreTrack ();
  ...
  for (iteratorCoreTrack = listCoreTrack.begin ();
       iteratorCoreTrack != listCoreTrack.end (); ++iteratorCoreTrack)
      {
        // save core track
                              ...
                              CalCoreBone * pCoreBone)
{
  ...
  const CalVector & translationBoneSpace =
    pCoreBone->getTranslationBoneSpace ();
  ...
  // write the rotation of the bone
  const CalQuaternion & rotationBoneSpace =
    pCoreBone->getRotationBoneSpace ();
  ...
  for (iteratorChildId = listChildId.begin ();
       iteratorChildId != listChildId.end (); ++iteratorChildId)
      {
        // write the child id
                                 ...
                                 CalCoreMaterial * pCoreMaterial)
{
  ...
  // write magic tag
  if (!CalPlatform::
      writeBytes (file, &Cal::MATERIAL_FILE_MAGIC,
                  sizeof (Cal::MATERIAL_FILE_MAGIC)))
      {
        CalError::setLastError (CalError::FILE_WRITING_FAILED, __FILE__,
                                __LINE__, strFilename);
        return false;
  ...
  // get the map vector
  std::vector < CalCoreMaterial::Map > &vectorMap =
    pCoreMaterial->getVectorMap ();
                             ...
                             CalCoreMesh * pCoreMesh)
{
  ...
  // write magic tag
  if (!CalPlatform::
      writeBytes (file, &Cal::MESH_FILE_MAGIC, sizeof (Cal::MESH_FILE_MAGIC)))
      {
        CalError::setLastError (CalError::FILE_WRITING_FAILED, __FILE__,
                                __LINE__, strFilename);
        return false;
  ...
  std::vector < CalCoreSubmesh * >&vectorCoreSubmesh =
  ...
  int submeshId;
  for (submeshId = 0; submeshId < (int) vectorCoreSubmesh.size ();
       ++submeshId)
      {
        // write the core submesh
        if (!saveCoreSubmesh
            (file, strFilename, vectorCoreSubmesh[submeshId]))
            {
              return false;
            }
      }
                                 ...
                                 CalCoreSkeleton * pCoreSkeleton)
{
  ...
  // write magic tag
  if (!CalPlatform::
      writeBytes (file, &Cal::SKELETON_FILE_MAGIC,
                  sizeof (Cal::SKELETON_FILE_MAGIC)))
      {
        CalError::setLastError (CalError::FILE_WRITING_FAILED, __FILE__,
                                __LINE__, strFilename);
        return false;
  ...
  if (!CalPlatform::
      writeInteger (file, pCoreSkeleton->getVectorCoreBone ().size ()))
      {
        CalError::setLastError (CalError::FILE_WRITING_FAILED, __FILE__,
                                __LINE__, strFilename);
        return false;
  ...
  // write all core bones
  int boneId;
  for (boneId = 0; boneId < (int) pCoreSkeleton->getVectorCoreBone ().size ();
       ++boneId)
      {
        // write the core bone
        if (!saveCoreBones
            (file, strFilename, pCoreSkeleton->getCoreBone (boneId)))
            {
              return false;
            }
      }
                                ...
                                CalCoreSubmesh * pCoreSubmesh)
{
  ...
  if (!CalPlatform::
      writeInteger (file, pCoreSubmesh->getCoreMaterialThreadId ()))
      {
        CalError::setLastError (CalError::FILE_WRITING_FAILED, __FILE__,
                                __LINE__, strFilename);
        return false;
  ...
  for (vertexId = 0; vertexId < (int) vectorVertex.size (); ++vertexId)
      {
        ...
        for (textureCoordinateId = 0;
             textureCoordinateId <
             (int) vectorvectorTextureCoordinate.size ();
             ++textureCoordinateId)
            {
              CalCoreSubmesh::TextureCoordinate & textureCoordinate =
                vectorvectorTextureCoordinate[textureCoordinateId][vertexId];
              ...
              if (!file)
                  {
                    CalError::setLastError (CalError::FILE_WRITING_FAILED,
        ...
        int influenceId;
        for (influenceId = 0;
             influenceId < (int) vertex.vectorInfluence.size ();
             ++influenceId)
            {
              CalCoreSubmesh::Influence & influence =
                vertex.vectorInfluence[influenceId];
        ...
        if (pCoreSubmesh->getSpringCount () > 0)
            {
              ...
              CalCoreSubmesh::PhysicalProperty & physicalProperty =
                vectorPhysicalProperty[vertexId];
  ...
  // write all springs
  int springId;
  for (springId = 0; springId < (int) pCoreSubmesh->getSpringCount ();
       ++springId)
      {
  ...
  for (int i = 0; i < pCoreTrack->getCoreKeyframeCount (); ++i)
      {
        ...
        if (!saveCoreKeyframe
            (file, strFilename, pCoreTrack->getCoreKeyframe (i)))
            {
              return false;
            }
      }
                                    ...
                                    CalCoreSkeleton * pCoreSkeleton)
{
  ...
  skeleton.SetAttribute ("NUMBONES",
                         pCoreSkeleton->getVectorCoreBone ().size ());
  ...
  for (boneId = 0; boneId < (int) pCoreSkeleton->getVectorCoreBone ().size ();
       ++boneId)
      {
        ...
        str.str ("");
        str << rotationQuad.x << " "
          << rotationQuad.y << " " << rotationQuad.z << " " << rotationQuad.w;
        ...
        const CalVector & localtranslationVector =
          pCoreBone->getTranslationBoneSpace ();

        str.str ("");
        str << localtranslationVector.x << " "
          << localtranslationVector.y << " " << localtranslationVector.z;
        ...
        const CalQuaternion & localrotationQuad =
          pCoreBone->getRotationBoneSpace ();

        str.str ("");
        str << localrotationQuad.x << " "
          << localrotationQuad.y << " "
          << localrotationQuad.z << " " << localrotationQuad.w;
                                     ...
                                     CalCoreAnimation * pCoreAnimation)
{
  ...
  for (iteratorCoreTrack = listCoreTrack.begin ();
       iteratorCoreTrack != listCoreTrack.end (); ++iteratorCoreTrack)
      {
        ...
        track.SetAttribute ("NUMKEYFRAMES",
                            pCoreTrack->getCoreKeyframeCount ());

        // save all core keyframes
        for (int i = 0; i < pCoreTrack->getCoreKeyframeCount (); ++i)
            {
              CalCoreKeyframe *pCoreKeyframe =
                pCoreTrack->getCoreKeyframe (i);
              ...
              TiXmlElement rotation ("ROTATION");
              const CalQuaternion & rotationQuad =
                pCoreKeyframe->getRotation ();

              str.str ("");
              str << rotationQuad.x << " "
                << rotationQuad.y << " "
                << rotationQuad.z << " " << rotationQuad.w;
                                ...
                                CalCoreMesh * pCoreMesh)
{
  ...
  for (submeshId = 0; submeshId < (int) vectorCoreSubmesh.size ();
       ++submeshId)
      {
        ...
        std::vector < CalCoreSubmesh::Face > &vectorFace =
          pCoreSubmesh->getVectorFace ();
        std::vector < CalCoreSubmesh::PhysicalProperty >
        ...
        for (vertexId = 0; vertexId < (int) vectorVertex.size (); ++vertexId)
            {
              ...
              vertex.SetAttribute ("NUMINFLUENCES",
                                   Vertex.vectorInfluence.size ());
              ...
              for (textureCoordinateId = 0;
                  ...
                  {
                    CalCoreSubmesh::TextureCoordinate & textureCoordinate =
                      vectorvectorTextureCoordinate[textureCoordinateId]
                      [vertexId];
                                    ...
                                    CalCoreMaterial * pCoreMaterial)
{
  ...
  str << (int) ambientColor.red << " "
    ...
    << (int) ambientColor.blue << " " << (int) ambientColor.alpha;
  ...
  str << (int) diffuseColor.red << " "
    ...
    << (int) diffuseColor.blue << " " << (int) diffuseColor.alpha;
  ...
  str << (int) specularColor.red << " "
    ...
    << (int) specularColor.blue << " " << (int) specularColor.alpha;
