...
CalSkeleton::CalSkeleton ():m_pCoreSkeleton (0)
{
}
...
void CalSkeleton::calculateState ()
{
  ...
  std::list < int >&listRootCoreBoneId =
    m_pCoreSkeleton->getListRootCoreBoneId ();

  std::list < int >::iterator iteratorRootBoneId;
  for (iteratorRootBoneId = listRootCoreBoneId.begin ();
       iteratorRootBoneId != listRootCoreBoneId.end (); ++iteratorRootBoneId)
      {
...
void CalSkeleton::clearState ()
{
  ...
  for (iteratorBone = m_vectorBone.begin ();
       iteratorBone != m_vectorBone.end (); ++iteratorBone)
      {
...
bool CalSkeleton::create (CalCoreSkeleton * pCoreSkeleton)
{
  ...
  std::vector < CalCoreBone * >&vectorCoreBone =
    pCoreSkeleton->getVectorCoreBone ();
  ...
  if ((iteratorBone = m_vectorBone.begin ()) != m_vectorBone.end ())
      {
        const CalVector & translation =
          (*iteratorBone)->getTranslationAbsolute ();
...
int CalSkeleton::getBonePointsStatic (float *pPoints)
{
  ...
  for (iteratorBone = m_vectorBone.begin ();
      ...
      {
        const CalVector & translation =
          (*iteratorBone)->getCoreBone ()->getTranslationAbsolute ();
        ...
        if (parentId != -1)
            {
              ...
              const CalVector & translation =
                (*iteratorBone)->getTranslationAbsolute ();
              const CalVector & translationParent =
                pParent->getTranslationAbsolute ();
        ...
        if (parentId != -1)
            {
              ...
              const CalVector & translation =
                (*iteratorBone)->getCoreBone ()->getTranslationAbsolute ();
              const CalVector & translationParent =
                pParent->getCoreBone ()->getTranslationAbsolute ();
