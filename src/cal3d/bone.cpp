...
CalBone::CalBone ():m_pCoreBone (0), m_pSkeleton (0)
{
}
...
void CalBone::blendState (float weight, const CalVector & translation,
...
void CalBone::calculateState ()
{
  ...
  for (iteratorChildId = m_pCoreBone->getListChildId ().begin ();
       iteratorChildId != m_pCoreBone->getListChildId ().end ();
       ++iteratorChildId)
      {
  ...
  if (m_accumulatedWeightAbsolute > 0.0f)
      {
        if (m_accumulatedWeight == 0.0f)
        ...
        else
            {
              ...
              float factor;
              factor =
                m_accumulatedWeightAbsolute / (m_accumulatedWeight +
...
{
  int boneId =
    m_pSkeleton->getCoreSkeleton ()->getCoreBoneId (getCoreBone ()->
                                                    getName ());
