...
CalRenderer::CalRenderer ():m_pSelectedSubmesh (0)
{
}
...
void CalRenderer::getAmbientColor (unsigned char *pColorBuffer)
{
  ...
  pCoreMaterial =
    m_pModel->getCoreModel ()->getCoreMaterial (m_pSelectedSubmesh->
                                                getCoreMaterialId ());
  if (pCoreMaterial == 0)
      {
...
int CalRenderer::getTangentSpaces (int mapId, float *pTangentSpaceBuffer)
{
  ...
  std::vector < std::vector < CalCoreSubmesh::TangentSpace >
    >&vectorvectorTangentSpace =
    m_pSelectedSubmesh->getCoreSubmesh ()->getVectorVectorTangentSpace ();

  // check if the map id is valid
  if ((mapId < 0) || (mapId >= (int) vectorvectorTangentSpace.size ())
      || !m_pSelectedSubmesh->isTangentsEnabled (mapId))
      {
        CalError::setLastError (CalError::INVALID_HANDLE, __FILE__, __LINE__);
        return -1;
      }
  ...
  if (m_pSelectedSubmesh->hasInternalData ())
      {
        ...
        std::vector < CalSubmesh::TangentSpace > &vectorTangentSpace =
          m_pSelectedSubmesh->getVectorVectorTangentSpace ()[mapId];
        ...
        memcpy (pTangentSpaceBuffer, &vectorTangentSpace[0],
                tangentSpaceCount * sizeof (CalCoreSubmesh::TangentSpace));

        return tangentSpaceCount;
      }

  // submesh does not handle the vertex data internally, so let the physique calculate it now
  return m_pModel->getPhysique ()->calculateTangentSpaces (m_pSelectedSubmesh,
                                                           mapId,
                                                           pTangentSpaceBuffer);
}
...
int CalRenderer::getNormals (float *pNormalBuffer)
{
  ...
  if (m_pSelectedSubmesh->hasInternalData ())
      {
        // get the normal vector of the submesh
        std::vector < CalVector > &vectorNormal =
          m_pSelectedSubmesh->getVectorNormal ();
        ...
        // copy the internal normal data to the provided normal buffer
        memcpy (pNormalBuffer, &vectorNormal[0],
                normalCount * sizeof (CalVector));

        return normalCount;
      }

  // submesh does not handle the vertex data internally, so let the physique calculate it now
  return m_pModel->getPhysique ()->calculateNormals (m_pSelectedSubmesh,
                                                     pNormalBuffer);
}
                                        ...
                                        float *pTextureCoordinateBuffer)
{
  ...
  std::vector < std::vector < CalCoreSubmesh::TextureCoordinate >
    ...
    m_pSelectedSubmesh->getCoreSubmesh ()->
  ...
  memcpy (pTextureCoordinateBuffer, &vectorvectorTextureCoordinate[mapId][0],
          textureCoordinateCount *
          sizeof (CalCoreSubmesh::TextureCoordinate));
  ...
  if (m_pSelectedSubmesh->hasInternalData ())
      {
        ...
        memcpy (pVertexBuffer, &vectorVertex[0],
                vertexCount * sizeof (CalVector));
...
int CalRenderer::getVerticesAndNormals (float *pVertexBuffer)
{
  ...
  if (m_pSelectedSubmesh->hasInternalData ())
      {
        ...
        for (int i = 0; i < vertexCount; ++i)
            {
              memcpy (&pVertexBuffer[0], &vectorVertex[i],
                      sizeof (CalVector));
              memcpy (&pVertexBuffer[3], &vectorNormal[i],
                      sizeof (CalVector));
              pVertexBuffer += 6;
            }
  ...
  // submesh does not handle the vertex data internally, so let the physique calculate it now
  return m_pModel->getPhysique ()->
    calculateVerticesAndNormals (m_pSelectedSubmesh, pVertexBuffer);
}
                                                 ...
                                                 int NumTexCoords)
{
        ...
        if ((NumTexCoords < 0) || (NumTexCoords > TextureCoordinateCount))
            {
              if (TextureCoordinateCount != 0)
                  {
                    CalError::setLastError (CalError::INVALID_HANDLE,
                                            __FILE__, __LINE__);
                    return -1;
                  }
            }
        ...
        if (TextureCoordinateCount == 0)
            {
              for (int vertexId = 0; vertexId < vertexCount; ++vertexId)
                  {
                    memcpy (&pVertexBuffer[0], &vectorVertex[vertexId],
                            sizeof (CalVector));
                    memcpy (&pVertexBuffer[3], &vectorNormal[vertexId],
                            sizeof (CalVector));
                    ...
                    for (int vertexId = 0; vertexId < vertexCount; ++vertexId)
                        {
                          ...
                          memcpy (&pVertexBuffer[3], &vectorNormal[vertexId],
                                  sizeof (CalVector));
                          memcpy (&pVertexBuffer[6],
                                  &vectorvectorTextureCoordinate[0][vertexId],
                                  sizeof (CalCoreSubmesh::TextureCoordinate));
                          pVertexBuffer += 8;
                        }
                  }
                    ...
                    for (int vertexId = 0; vertexId < vertexCount; ++vertexId)
                        {
                          ...
                          for (int mapId = 0; mapId < NumTexCoords; ++mapId)
                              {
                                memcpy (&pVertexBuffer[0],
                                        &vectorvectorTextureCoordinate[mapId]
                                        [vertexId],
                                        sizeof (CalCoreSubmesh::
                                                TextureCoordinate));
                                pVertexBuffer += 2;
                              }
                        }
  ...
  // submesh does not handle the vertex data internally, so let the physique calculate it now
  return m_pModel->getPhysique ()->
    calculateVerticesNormalsAndTexCoords (m_pSelectedSubmesh, pVertexBuffer,
                                          NumTexCoords);
}
