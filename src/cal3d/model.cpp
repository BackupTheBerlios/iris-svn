...
CalModel::CalModel ():m_pCoreModel (0), m_pSkeleton (0), m_pMixer (0), m_pPhysique (0),
...
bool CalModel::create (CalCoreModel * pCoreModel)
{
  ...
  if (!m_pMorphTargetMixer->create (this))
    return false;
...
bool CalModel::detachMesh (int coreMeshId)
{
  ...
  for (iteratorMesh = m_vectorMesh.begin ();
       iteratorMesh != m_vectorMesh.end (); ++iteratorMesh)
      {
        // get the mesh
        CalMesh *pMesh;
        pMesh = *iteratorMesh;
...
void CalModel::update (float deltaTime)
{
  ...
  std::vector < CalMesh * >::iterator iteratorVectorMesh =
    m_vectorMesh.begin ();
  // m_pMorpher->update(...);
