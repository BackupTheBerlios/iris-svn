...
CalCoreBone::CalCoreBone ():m_pCoreSkeleton (0), m_parentId (-1),
m_userData (0)
{
}
...
void CalCoreBone::calculateState ()
{
  ...
  for (iteratorChildId = m_listChildId.begin ();
       iteratorChildId != m_listChildId.end (); ++iteratorChildId)
      {
  ...
  for (meshId = 0; meshId < pCoreModel->getCoreMeshCount (); ++meshId)
      {
        ...
        int submeshId;
        for (submeshId = 0; submeshId < pCoreMesh->getCoreSubmeshCount ();
             submeshId++)
            {
              CalCoreSubmesh *pCoreSubmesh =
                pCoreMesh->getCoreSubmesh (submeshId);
              ...
              if (pCoreSubmesh->getSpringCount () == 0)
                  {
                    ...
                    for (size_t vertexId = 0; vertexId < vectorVertex.size ();
                         ++vertexId)
                        {
                          for (size_t influenceId = 0;
                               influenceId <
                               vectorVertex[vertexId].vectorInfluence.size ();
                               ++influenceId)
                              {
                                if (vectorVertex[vertexId].
                                    vectorInfluence[influenceId].boneId ==
                                    boneId)
                                    {
                                      int planeId;
                                      for (planeId = 0; planeId < 6;
                                           ++planeId)
                                          {
                                            if (m_boundingBox.plane[planeId].
                                                eval (vectorVertex[vertexId].
                                                      position) < 0.0f)
                                                {
                                                  m_boundingBox.
                                                    plane[planeId].
                                                    setPosition (vectorVertex
                                                                 [vertexId].
                                                                 position);
                                                  m_boundingPosition[planeId]
                                                    =
                                                    vectorVertex[vertexId].
                                                    position;
                                                }
                                          }
                                    }
