...
CalCoreTrack::CalCoreTrack ():m_coreBoneId (-1)
{
}
...
bool CalCoreTrack::addCoreKeyframe (CalCoreKeyframe * pCoreKeyframe)
{
  ...
  while (idx > 0
         && m_keyframes[idx]->getTime () < m_keyframes[idx - 1]->getTime ())
      {
...
bool CalCoreTrack::getState (float time, CalVector & translation,
                             CalQuaternion & rotation)
{
  ...
  blendFactor =
    (time -
     pCoreKeyframeBefore->getTime ()) / (pCoreKeyframeAfter->getTime () -
                                         pCoreKeyframeBefore->getTime ());
...
std::vector <
  CalCoreKeyframe * >::iterator CalCoreTrack::getUpperBound (float time)
{
