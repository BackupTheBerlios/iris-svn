...
CalSpringSystem::CalSpringSystem ():m_pModel (0)
{
}
...
void CalSpringSystem::calculateForces (CalSubmesh * pSubmesh, float deltaTime)
{
  ...
  // get the vertex vector of the submesh
  std::vector < CalSubmesh::PhysicalProperty > &vectorPhysicalProperty =
  ...
  for (vertexId = 0; vertexId < (int) vectorVertex.size (); ++vertexId)
      {
        ...
        CalSubmesh::PhysicalProperty & physicalProperty =
          vectorPhysicalProperty[vertexId];

        // get the physical property of the core vertex
        CalCoreSubmesh::PhysicalProperty & corePhysicalProperty =
          vectorCorePhysicalProperty[vertexId];
...
//      physicalProperty.force.set(0.0f, 0.0f /*0.5f*/, corePhysicalProperty.weight * -98.1f);
              physicalProperty.force.set (0.0f, 0.5f,
                                          corePhysicalProperty.weight *
                                          -98.1f);
            }
      }
}
                                         ...
                                         float deltaTime)
{
  ...
  for (vertexId = 0; vertexId < (int) vectorVertex.size (); ++vertexId)
      {
        ...
        if (corePhysicalProperty.weight > 0.0f)
            {
              // do the Verlet step
              physicalProperty.position +=
                (position - physicalProperty.positionOld) * 0.99f +
                physicalProperty.force / corePhysicalProperty.weight *
                deltaTime * deltaTime;
            }
        else
            {
  ...
  std::vector < CalCoreSubmesh::Spring > &vectorSpring =
    pSubmesh->getCoreSubmesh ()->getVectorSpring ();
  ...
  for (iterationCount = 0; iterationCount < ITERATION_COUNT; ++iterationCount)
      {
        ...
        for (iteratorSpring = vectorSpring.begin ();
             iteratorSpring != vectorSpring.end (); ++iteratorSpring)
            {
              // get the spring
              ...
              CalVector distance;
              distance =
                vectorVertex[spring.vertexId[1]] -
                vectorVertex[spring.vertexId[0]];
              ...
              if (length > 0.0f)
                  {
                    ...
                    if (vectorCorePhysicalProperty[spring.vertexId[0]].
                        weight > 0.0f)
                        {
                          factor[0] /= 2.0f;
                          factor[1] /= 2.0f;
                    ...
                    if (vectorCorePhysicalProperty[spring.vertexId[1]].
                        weight <= 0.0f)
                        {
                          factor[0] *= 2.0f;
                          factor[1] = 0.0f;
                    ...
                    vectorPhysicalProperty[spring.vertexId[0]].position =
                      vectorVertex[spring.vertexId[0]];

                    vectorVertex[spring.vertexId[1]] -= distance * factor[1];
                    vectorPhysicalProperty[spring.vertexId[1]].position =
                      vectorVertex[spring.vertexId[1]];
                  }
            }
      }
        ...
        for (iteratorSubmesh = vectorSubmesh.begin ();
             iteratorSubmesh != vectorSubmesh.end (); ++iteratorSubmesh)
            {
              ...
              if ((*iteratorSubmesh)->getCoreSubmesh ()->getSpringCount () > 0
                  && (*iteratorSubmesh)->hasInternalData ())
                  {
