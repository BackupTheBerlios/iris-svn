...
CalCoreSubmesh::CalCoreSubmesh ():m_coreMaterialThreadId (0), m_lodCount (0)
{
}
...
void CalCoreSubmesh::destroy ()
{
  ...
  std::vector <
    CalCoreSubMorphTarget * >::iterator iteratorCoreSubMorphTarget;
...
{
  if ((mapId < 0) || (mapId >= (int) m_vectorTangentsEnabled.size ()))
    return false;
...
void CalCoreSubmesh::UpdateTangentVector (int v0, int v1, int v2, int mapId)
{
  ...
  std::vector < CalCoreSubmesh::TextureCoordinate > &vtex =
    m_vectorvectorTextureCoordinate[mapId];
...
bool CalCoreSubmesh::enableTangents (int mapId, bool enabled)
{
  ...
  int tangentId;
  for (tangentId = 0;
       tangentId < (int) m_vectorvectorTangentSpace[mapId].size ();
       tangentId++)
      {
        m_vectorvectorTangentSpace[mapId][tangentId].tangent =
          CalVector (0.0f, 0.0f, 0.0f);
  ...
  for (faceId = 0; faceId < (int) m_vectorFace.size (); faceId++)
      {
        UpdateTangentVector (m_vectorFace[faceId].vertexId[0],
                             m_vectorFace[faceId].vertexId[1],
                             m_vectorFace[faceId].vertexId[2], mapId);
        UpdateTangentVector (m_vectorFace[faceId].vertexId[1],
                             m_vectorFace[faceId].vertexId[2],
                             m_vectorFace[faceId].vertexId[0], mapId);
        UpdateTangentVector (m_vectorFace[faceId].vertexId[2],
                             m_vectorFace[faceId].vertexId[0],
                             m_vectorFace[faceId].vertexId[1], mapId);
      }
...
std::vector < CalCoreSubmesh::PhysicalProperty >
  &CalCoreSubmesh::getVectorPhysicalProperty ()
{
...
std::vector < std::vector < CalCoreSubmesh::TextureCoordinate >
  >&CalCoreSubmesh::getVectorVectorTextureCoordinate ()
{
...
std::vector < std::vector < CalCoreSubmesh::TangentSpace >
  >&CalCoreSubmesh::getVectorVectorTangentSpace ()
{
...
bool CalCoreSubmesh::reserve (int vertexCount, int textureCoordinateCount,
                              int faceCount, int springCount)
{
  ...
  for (textureCoordinateId = 0; textureCoordinateId < textureCoordinateCount;
       ++textureCoordinateId)
      {
        m_vectorvectorTextureCoordinate[textureCoordinateId].
          reserve (vertexCount);
        m_vectorvectorTextureCoordinate[textureCoordinateId].
          resize (vertexCount);
        ...
        if (m_vectorTangentsEnabled[textureCoordinateId])
            {
              m_vectorvectorTangentSpace[textureCoordinateId].
                reserve (vertexCount);
              m_vectorvectorTangentSpace[textureCoordinateId].
                resize (vertexCount);
            }
...
bool CalCoreSubmesh::setTangentSpace (int vertexId, int textureCoordinateId,
                                      const CalVector & tangent,
                                      float crossFactor)
{
  if ((vertexId < 0) || (vertexId >= (int) m_vectorVertex.size ()))
    return false;
  if ((textureCoordinateId < 0)
      || (textureCoordinateId >=
          (int) m_vectorvectorTextureCoordinate.size ()))
    return false;
  if (!m_vectorTangentsEnabled[textureCoordinateId])
    return false;

  m_vectorvectorTangentSpace[textureCoordinateId][vertexId].tangent = tangent;
  m_vectorvectorTangentSpace[textureCoordinateId][vertexId].crossFactor =
    crossFactor;
  return true;
}
...
bool CalCoreSubmesh::setPhysicalProperty (int vertexId,
                                          ...
                                          physicalProperty)
{
  if ((vertexId < 0) || (vertexId >= (int) m_vectorPhysicalProperty.size ()))
    return false;
...
{
  if ((springId < 0) || (springId >= (int) m_vectorSpring.size ()))
    return false;
                                           ...
                                           textureCoordinate)
{
  ...
  if ((vertexId < 0)
      || (vertexId >=
          (int) m_vectorvectorTextureCoordinate[textureCoordinateId].size ()))
    return false;

  m_vectorvectorTextureCoordinate[textureCoordinateId][vertexId] =
    textureCoordinate;
...
int CalCoreSubmesh::addCoreSubMorphTarget (CalCoreSubMorphTarget *
                                           pCoreSubMorphTarget)
{
...
std::vector <
  CalCoreSubMorphTarget * >&CalCoreSubmesh::getVectorCoreSubMorphTarget ()
{
