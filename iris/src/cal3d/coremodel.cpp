...
CalCoreModel::CalCoreModel ():m_pCoreSkeleton (0), m_userData (0)
{
}
...
int CalCoreModel::addCoreMorphAnimation (CalCoreMorphAnimation *
                                         pCoreMorphAnimation)
{
...
bool CalCoreModel::createCoreMaterialThread (int coreMaterialThreadId)
{
  ...
  m_mapmapCoreMaterialThread.
    insert (std::make_pair (coreMaterialThreadId, mapCoreMaterialThreadId));
...
void CalCoreModel::destroy ()
{
  ...
  for (iteratorCoreMaterial = m_vectorCoreMaterial.begin ();
       iteratorCoreMaterial != m_vectorCoreMaterial.end ();
       ++iteratorCoreMaterial)
      {
  ...
  for (iteratorCoreMesh = m_vectorCoreMesh.begin ();
       iteratorCoreMesh != m_vectorCoreMesh.end (); ++iteratorCoreMesh)
      {
  ...
  for (iteratorCoreAnimation = m_vectorCoreAnimation.begin ();
       iteratorCoreAnimation != m_vectorCoreAnimation.end ();
       ++iteratorCoreAnimation)
      {
  ...
  // destroy all core animations
  std::vector <
    CalCoreMorphAnimation * >::iterator iteratorCoreMorphAnimation;
  for (iteratorCoreMorphAnimation = m_vectorCoreMorphAnimation.begin ();
       iteratorCoreMorphAnimation != m_vectorCoreMorphAnimation.end ();
       ++iteratorCoreMorphAnimation)
      {
...
CalCoreMorphAnimation *CalCoreModel::
getCoreMorphAnimation (int coreMorphAnimationId)
{
  if ((coreMorphAnimationId < 0)
      || (coreMorphAnimationId >= (int) m_vectorCoreMorphAnimation.size ()))
      {
        CalError::setLastError (CalError::INVALID_HANDLE, __FILE__, __LINE__);
        return 0;
      }
...
{
  if ((coreMaterialId < 0)
      || (coreMaterialId >= (int) m_vectorCoreMaterial.size ()))
      {
        CalError::setLastError (CalError::INVALID_HANDLE, __FILE__, __LINE__);
        return 0;
      }
                                     ...
                                     int coreMaterialSetId)
{
  ...
  iteratorCoreMaterialThread =
    m_mapmapCoreMaterialThread.find (coreMaterialThreadId);
  ...
  std::map < int, int >&coreMaterialThread =
    (*iteratorCoreMaterialThread).second;
                                      ...
                                      int coreAnimationId)
{
  ...
  if (!CalSaver::
      saveCoreAnimation (strFilename, m_vectorCoreAnimation[coreAnimationId]))
      {
        return false;
      }
                                     ...
                                     int coreMaterialId)
{
  ...
  // save the core animation
  if (!CalSaver::
      saveCoreMaterial (strFilename, m_vectorCoreMaterial[coreMaterialId]))
      {
        return false;
      }
                                      ...
                                      int coreMaterialId)
{
  ...
  coreMaterialThread.
    insert (std::make_pair (coreMaterialSetId, coreMaterialId));
