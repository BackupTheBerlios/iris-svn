...
// Functions
  ...
  void writeDocument()
  {
     ...
     if(!fp)
     {
            pDebug.Log("Unable to saving configuration file", __FILE__, __LINE__,
                  LEVEL_WARNING);
            return;
     }
     
     sprintf(ln,"<CONFIG>\n");
     fwrite(ln,1,strlen(ln),fp);
     sprintf(ln,"\t<UO>\n");
     fwrite(ln,1,strlen(ln),fp);
     sprintf(ln,"\t\t<MULPATH>%s</MULPATH>\n",mulpath.c_str());
     fwrite(ln,1,strlen(ln),fp);
     sprintf(ln,"\t\t<MINX>%d</MINX>\n",minx);     
     fwrite(ln,1,strlen(ln),fp);
     sprintf(ln,"\t\t<MINY>%d</MINY>\n",miny);     
     fwrite(ln,1,strlen(ln),fp);
     sprintf(ln,"\t\t<MAXX>%d</MAXX>\n",maxx);     
     fwrite(ln,1,strlen(ln),fp);
     sprintf(ln,"\t\t<MAXY>%d</MAXY>\n",maxy);     
     fwrite(ln,1,strlen(ln),fp);
     sprintf(ln,"\t\t<OUTPUT>%s</OUTPUT>\n",output.c_str());
     fwrite(ln,1,strlen(ln),fp);
     sprintf(ln,"\t\t<DETAIL>%d</DETAIL>\n",detail);     
     fwrite(ln,1,strlen(ln),fp);
     sprintf(ln,"\t</UO>\n");
     fwrite(ln,1,strlen(ln),fp);
     sprintf(ln,"</CONFIG>\n");
