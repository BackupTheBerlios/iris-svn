   ...
   ============================================================*/
{
	...
	XML::Node* meta_node = node->findNode("event");
	if (meta_node) {
	...
	if( main.GetSize() == 2 )
	{
		...
		if( header == GetType() )
		{
			CTokenizer tok( data.Left( data.GetLength() - 1 ) ); // Strip the ';'
   ...
   ============================================================*/
{
	...
	if (bBegin) {
		CRect rect = GetRect();
		str.Format("<control class='%s' name='%s' left='%d' top='%d' width='%d' height='%d'>\n", 
			GetType(), GetName(), rect.left, rect.top, rect.Width(), rect.Height());
		tmp.Format(" <meta selected='%d' freezed='%d' visible='%d'/>", IsSelected(), IsFreezed(), IsVisible());
		str += tmp;
	} else {
		str = "</control>";
	}
...
CString CDiagramEntity::GetEventHandler() const
{
	return "";
}
   ...
   ============================================================*/
{
	...
	if( menu.CreatePopupMenu() )
	{
		...
		menu.AppendMenu( MF_STRING, CMD_SHOW_CODE, _T( "Code View" ) );
		menu.AppendMenu( MF_SEPARATOR );
