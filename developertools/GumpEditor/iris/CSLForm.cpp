...
//  ...
//	csl->addFunc(moduel, "control_getid (const name)", api_control_getid);
...
ZString api_gui_addform (ZCsl* aCsl)
{
	...
	std::string gfm_file = aCsl->get("gfm_file");
	bool is_container = argCount > 3 ? aCsl->get("is_container").asInt() : false;
