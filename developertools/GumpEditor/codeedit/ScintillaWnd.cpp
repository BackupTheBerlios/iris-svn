...
// @rvalue void | not used
//
BOOL CScintillaWnd::FindNextBookmark(long lLine)
{
	if (lLine < 0) lLine = GetCurrentLine();
	lLine = SendMessage(SCI_MARKERNEXT, lLine, 1);
	if (lLine < 0) return FALSE;
...
// @rvalue void | not used
//
BOOL CScintillaWnd::FindPreviousBookmark(long lLine)
{
	if (lLine < 0) lLine = GetCurrentLine()-2;
	lLine = SendMessage(SCI_MARKERPREVIOUS, lLine, 1);	
	if (lLine < 0) return FALSE;
	SendMessage(SCI_GOTOLINE, lLine,0);
	return true;
}
