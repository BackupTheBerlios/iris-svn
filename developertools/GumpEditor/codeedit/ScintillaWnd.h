...
// @cmember Find next bookmark
   virtual BOOL FindNextBookmark(long lLine=-1);
// @cmember Find previous bookmark
   virtual BOOL FindPreviousBookmark(long lLine=-1);
// @cmember goto line
...
// @cmember replace all in buffer or selection
   ...
   long GetLineCount() { return SendMessage(SCI_GETLINECOUNT); }
...
#ifdef _DEBUG
#define STR_LEXERDLL     _T("scilexerd.dll")
#else
