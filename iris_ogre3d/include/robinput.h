...
class cInput : public cListenable {
public :
	...
	void	MouseStateUpdate (const int iAbsX,const int iAbsY,const int iRelZ,const bool b1,const bool b2,const bool b3);
	
	unsigned char	KeyConvertWin		(const int iVKey,const bool bRight);
	unsigned char	KeyConvertSDL		(const int iVKey);
	unsigned char	KeyConvertSDLMouse	(const int iMButton);
	unsigned char	KeyConvertOgre		(const int iKeyCode);
	unsigned char	KeyConvertOgreMouse	(const int iButtonID);
	unsigned char	GetNamedKey			(const char* szName);
	const char*		GetKeyName			(const unsigned char iKey);

	void	KeyDown			(const unsigned char iKey,const char sChar=0);
	...
	enum {
		...
		kListenerEvent_Char, // when a keydown also triggers a letter being typed
	};
