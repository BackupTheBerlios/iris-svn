...
public:
	...
	std::vector<Ogre::PanelOverlayElement*> mlRect;
	bool	mbNeedGfxUpdate; // update does not mean recreation, just adjust size,pos,alpha,hue....
	bool	mbMustCreateGfx; // recreateion is neccessary to for bring to front and init
	void	AddToRenderQueue		();
	void	RemoveFromRenderQueue	();
	void 	BringToFront			();
	void	GUIHandlerStep			(GumpHandler *gumps); // called from GUIHandler, will call the overridable DrawStep
	virtual void CreateGFX			(GumpHandler *gumps); // override me
	virtual void DrawStep			(GumpHandler *gumps); // override me (to be used for animation, e.g. blinking cursor, glowing button, fade)
	virtual void DestroyGFX			();					  // override me
	
	Ogre::PanelOverlayElement*	CreateRect		();
	void						DestroyRect		(Ogre::PanelOverlayElement* panel);
	
	void UpdateRectHued	(Ogre::PanelOverlayElement* panel,  int x, int y, int width, int height, Texture * texture, int hue, unsigned char alpha, bool swapy = false );
	void UpdateRect		(Ogre::PanelOverlayElement* panel,  int x, int y, int width, int height, Texture * texture, unsigned char alpha, bool swapy = false );
	void UpdateRect		(Ogre::PanelOverlayElement* panel,  int x, int y, int width, int height, Texture * texture );
	void UpdateRect		(Ogre::PanelOverlayElement* panel,  int x, int y, Texture * texture );
	
	virtual void Draw					(GumpHandler *gumps); // obsolete with ogre, use DrawStep for periodic change
	void DrawRectHued( int x, int y, int width, int height, Texture * texture, int hue, unsigned char alpha, bool swapy = false ); // obsolete
	void DrawRect( int x, int y, int width, int height, Texture * texture, unsigned char alpha, bool swapy = false ); // obsolete
	void DrawRect( int x, int y, int width, int height, Texture * texture ); // obsolete
	void DrawRect( int x, int y, Texture * texture ); // obsolete
