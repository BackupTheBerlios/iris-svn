...
*/
	...
	try
	{
		...
		mySDLscreen = new SDLScreen();
		// Initialize Input(SDL) Event
		SDLEvent *mySDLevent = new SDLEvent();
		
		// register mySDLevent for events from ogre
		cInput::GetSingleton().RegisterListener(mySDLevent,(int)SDLEvent::kEventBinding_InputEvent);
		cTimer::GetSingletonPtr()->RegisterFrameIntervalListener(mySDLevent,0,(int)SDLEvent::kEventBinding_EveryFrame);
		...
		while ( ogrewrapper.mbAlive && !mySDLevent->GetStatus() )  { 
			// Handle events in the queue
			
			// mySDLevent->PollEvent(); // replaced by ogre event listener
			kGame->Handle();
			kGame->RenderScene();
			ogrewrapper.DrawOneFrame();
