...
{ PROFILE
	...
	// reaktivate when ogre gui port is done
...
{ PROFILE
	...
	// mousepicking
	static Ogre::WireBoundingBox* pBBox = 0;
	if (!(cInput::bKeys[cInput::kkey_mouse1] && cInput::bKeys[cInput::kkey_mouse2])) {
		cOgreWrapper& ogrewrapper = cOgreWrapper::GetSingleton();
		Ogre::RaySceneQuery* pRaySceneQuery = ogrewrapper.mpRaySceneQuery;
		pRaySceneQuery->setRay(ogrewrapper.mCamera->getCameraToViewportRay(
			Ogre::Real(cInput::iMouse[0]) / Ogre::Real(ogrewrapper.mViewport->getActualWidth()),
			Ogre::Real(cInput::iMouse[1]) / Ogre::Real(ogrewrapper.mViewport->getActualHeight()) ));
		pRaySceneQuery->setSortByDistance(true);
		Ogre::RaySceneQueryResult& res = pRaySceneQuery->execute();
		
		bool debugprint = false;
		if (debugprint) printf("RaySceneQuery hit %d times\n",res.size());
		
		if (!pBBox) {
			pBBox = new Ogre::WireBoundingBox();
			cOgreWrapper::GetSingleton().mSceneMgr->getRootSceneNode()->attachObject(pBBox);
			pBBox->setMaterial("BaseWhiteNoLighting");
		}
		
		Ogre::MovableObject* found = 0;
		float fFoundDist;
		float fHitDist;
		
		for (int i=0;i<res.size();++i) {
			cOgreUserObjectWrapper* pUserObject = static_cast<cOgreUserObjectWrapper*>(res[i].movable->getUserObject());
			if (res[i].worldFragment) { if (debugprint) printf(" %d: world fragment !\n",i); continue; }
			if (!pUserObject) { if (debugprint) printf(" %d:no userobject\n",i); continue; }
			
			// so far was only bounds check, now check for real hit with mpEntity and terrain manual object
			// TODO : so far only bounds check, check for real hit with mpEntity and terrain manual object
			if (pUserObject->mpMapBlock3D && !pUserObject->mpStaticObject) { if (debugprint) printf(" %d: mpMapBlock3D\n",i); continue; }
			if (!pUserObject->mpEntity) continue;
			
			if (pUserObject->mpCharacter)		{ if (debugprint) printf(" %d: mpCharacter\n",i); }
			if (pUserObject->mpDynamicObject)	{ if (debugprint) printf(" %d: mpDynamicObject\n",i); }
			if (pUserObject->mpStaticObject)	{ if (debugprint) printf(" %d: mpStaticObject\n",i); }
			
			if (!ogrewrapper.RayEntityQuery(&pRaySceneQuery->getRay(),pUserObject->mpEntity,&fHitDist)) continue;
			if (!found || fHitDist < fFoundDist) {
				found = res[i].movable;
				fFoundDist = fHitDist;
			}
		}
		...
		RaySceneQueryResultEntry :
			Real 				distance 	/// Distance along the ray.
			MovableObject * 	movable		/// The movable, or NULL if this is not a movable result.
		*/
	} else {
		if (pBBox->isVisible()) pBBox->setVisible(false);
	}
	...
	if (1) {
		if (1) {
			// ghoulsblade : currently left&right are wrong, as the "right" (as in UO) way would be to mirror all 3d-gfx along the x axis, but i haven't done that yet
			if (cInput::bKeys[cInput::kkey_left])	Walk_Simple( WALK_RIGHT );
			if (cInput::bKeys[cInput::kkey_right])	Walk_Simple( WALK_LEFT );
		} else {
			if (cInput::bKeys[cInput::kkey_left])	Walk_Simple( WALK_LEFT );
			if (cInput::bKeys[cInput::kkey_right])	Walk_Simple( WALK_RIGHT );
		}
		if (cInput::bKeys[cInput::kkey_up])		Walk_Simple( WALK_FORWARD );
		if (cInput::bKeys[cInput::kkey_down]) {
...
{ PROFILE
	//cOgreWrapper::GetSingleton().SetCamera(Ogre::Vector3(-( x % 8 ),-( y % 8 ),-z));
	//cOgreWrapper::GetSingleton().CameraLookAt(Ogre::Vector3(0,0,--100));
...
{ PROFILE
	...
	// ghoulsblade : deaktivated, to be replaced by ogre input system in src/Game.cpp
	return;
	
	/* ghoulsblade : was called from Game::Handle() : 
		if ( button_right )
		{
			MoveToMouse();
		}
