...
const char* cStaticModel::GetOgreMeshName () {PROFILE
	...
	if (!mbHasBeenRendered) {
		mbHasBeenRendered = true;
		
		int iMaterialChangeCounter = 0;
		int iCurTexture = 0;
		int iCurFace,i,vertexcount;
		cStaticModelFace *face;
		cStaticModelNode *node;
		msOgreMeshName = cOgreWrapper::GetUniqueName();
		const char* szMaterialName = 0;
		// Texture->GetModelMaterial() 
	...
	return msOgreMeshName.c_str();
}

// TODO : ghoulsblade : obsolete, replaced by ogre
