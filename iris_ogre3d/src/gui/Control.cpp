...
Control::Control()
{PROFILE
	mbNeedGfxUpdate = false;
	mbMustCreateGfx = false;
	x = 0;
	y = 0;
	z = 0;
	focus = 0;
...
void Control::AddToRenderQueue		() {PROFILE
	//printf("Control::AddToRenderQueue\n");
	mbMustCreateGfx = true;
	mbNeedGfxUpdate = true;
}

void Control::RemoveFromRenderQueue	() {PROFILE
	//printf("Control::RemoveFromRenderQueue\n");
	DestroyGFX();
	mbMustCreateGfx = false;
}

void Control::BringToFront	() {PROFILE
	RemoveFromRenderQueue();
	AddToRenderQueue();
}

void Control::GUIHandlerStep	(GumpHandler *gumps) {PROFILE
	if (mbMustCreateGfx) {
		CreateGFX(gumps);
		mbMustCreateGfx = false;
	}
...
void Control::DrawStep (GumpHandler * gumps) {PROFILE
	// todo : set alpha for overlay for fade, as Control::Draw() is not called every frame anymore..
	if (mbNeedGfxUpdate) { /* update... */ }
}

Ogre::PanelOverlayElement*	Control::CreateRect		() {PROFILE
	Ogre::PanelOverlayElement* panel = static_cast<Ogre::PanelOverlayElement*>(Ogre::OverlayManager::getSingleton().createOverlayElement("Panel",cOgreWrapper::GetUniqueName()));
	cOgreWrapper::GetSingleton().mpOverlayGUI->add2D(panel);
...
void	Control::DestroyRect	(Ogre::PanelOverlayElement* panel) {PROFILE
	if (!panel) return;
	cOgreWrapper::GetSingleton().mpOverlayGUI->remove2D(panel);
	Ogre::OverlayManager::getSingleton().destroyOverlayElement(panel);
}

void Control::UpdateRectHued	(Ogre::PanelOverlayElement* panel,  int x, int y, int width, int height, Texture * texture, int hue, unsigned char alpha, bool swapy ) {PROFILE
	// TODO : hue not yet implemented
	UpdateRect(panel,x,y,width,height,texture,alpha,swapy);
}

void Control::UpdateRect		(Ogre::PanelOverlayElement* panel,  int x, int y, int width, int height, Texture * texture, unsigned char alpha, bool swapy ) {PROFILE
	if (!texture) { printf("Control::CreateRect() texture=0\n"); panel->hide(); return; }
	if ((texture->GetWidth () <= 0) || (texture->GetHeight () <= 0)) { 
		printf("Control::CreateRect() texture empty\n"); 
		panel->hide(); 
		return; 
	}
	...
	/*
	int screen_cy = Config::GetHeight();
	int y1 = screen_cy - 1 - (y);
	int y2 = screen_cy - 1 - (y + height);
	...
	*/
	...
	// glBindTexture (GL_TEXTURE_2D, texture->GetGLTex ()); // replaced by ogre
	panel->setMaterialName(texture->GetGUIMaterial());
	//panel->setMaterialName("BaseWhiteNoLighting");
	
	// TODO : SET ALPHA, currently not supported... : glColor4f (1.0, 1.0, 1.0, (float) alpha / 255.0);
	// printf("Control::CreateRect() %d,%d,%d,%d\n",x,y,width,height);

	panel->setUV(0,0,u,v);
	panel->setPosition(x,y);
	panel->setDimensions(width,height);
}
void Control::UpdateRect		(Ogre::PanelOverlayElement* panel,  int x, int y, int width, int height, Texture * texture ) {PROFILE
	UpdateRect (panel,x, y, width, height, texture, 255);
}
void Control::UpdateRect		(Ogre::PanelOverlayElement* panel,  int x, int y, Texture * texture ) {PROFILE
	UpdateRect (panel,x, y, 0, 0, texture);
}
...
{PROFILE
  ...
  if (height <= 0)
	...
	// TODO : ghoulsblade : port me to ogre (DrawStep,.. see gui/Image.cpp)
  /*
...
{PROFILE
	printf("WARNING ! Control::GetTiledGump called, which is not implemented !\n");
