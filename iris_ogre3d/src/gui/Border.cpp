...
void Border::DestroyGFX		() {PROFILE Control::DestroyGFX();}
void Border::DrawStep		(GumpHandler *gumps) {PROFILE
	if (!mbNeedGfxUpdate)  return;
		
	
  if (!_gump) {
	  for (int i=0;i<9;++i)  mlRect[i]->hide();
    return;
  }
  ...
  UpdateRect(mlRect[0],img_x[1], img_y[1], 0, 0, textures[4], alpha);

  // Draw Corners
  UpdateRect(mlRect[1],img_x[0], img_y[0], 0, 0, textures[0], alpha);
  UpdateRect(mlRect[2],img_x[2], img_y[0], 0, 0, textures[2], alpha);
  UpdateRect(mlRect[3],img_x[0], img_y[2], 0, 0, textures[6], alpha);
  UpdateRect(mlRect[4],img_x[2], img_y[2], 0, 0, textures[8], alpha);

  // Draw Borders
  UpdateRect(mlRect[5],img_x[1], img_y[0], 0, 0, textures[1], alpha);
  UpdateRect(mlRect[6],img_x[1], img_y[2], 0, 0, textures[7], alpha);

  UpdateRect(mlRect[7],img_x[0], img_y[1], 0, 0, textures[3], alpha);
  UpdateRect(mlRect[8],img_x[2], img_y[1], 0, 0, textures[5], alpha);
}
